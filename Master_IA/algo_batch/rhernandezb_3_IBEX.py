# -*- coding: utf-8 -*-
"""
Created on Tue Aug 24 11:05:13 2021
...
ah_ibex = ApiHandler(market = "IBEX")
...
df_medias["short_BKT"] = MME(df_holdeo_sin_na.iloc[:,0],50)
df_medias["long_BKT"] = MME(df_holdeo_sin_na.iloc[:,0],100)
df_medias["short_CLNX"] = MME(df_holdeo_sin_na.iloc[:,1],50)
df_medias["long_CLNX"] = MME(df_holdeo_sin_na.iloc[:,1],100)
df_medias["short_ENG"] = MME(df_holdeo_sin_na.iloc[:,2],50)
df_medias["long_ENG"] = MME(df_holdeo_sin_na.iloc[:,2],100)
df_medias["short_REE"] = MME(df_holdeo_sin_na.iloc[:,3],50)
df_medias["long_REE"] = MME(df_holdeo_sin_na.iloc[:,3],100)
df_medias["short_TEF"] = MME(df_holdeo_sin_na.iloc[:,4],50)
df_medias["long_TEF"] =MME(df_holdeo_sin_na.iloc[:,4],100)

df_medias["signals_BKT"] = np.where(df_medias["short_BKT"] > df_medias["long_BKT"], 1, 0)
df_medias["signals_CLNX"] = np.where(df_medias["short_CLNX"] > df_medias["long_CLNX"], 1, 0)
df_medias["signals_ENG"] = np.where(df_medias["short_ENG"] > df_medias["long_ENG"], 1, 0)
df_medias["signals_REE"] = np.where(df_medias["short_REE"] > df_medias["long_REE"], 1, 0)
df_medias["signals_TEF"] = np.where(df_medias["short_TEF"] > df_medias["long_TEF"], 1, 0)

df_medias["position_BKT"] = df_medias["signals_BKT"].diff()
df_medias["position_CLNX"] = df_medias["signals_CLNX"].diff()
df_medias["position_ENG"] = df_medias["signals_ENG"].diff()
df_medias["position_REE"] = df_medias["signals_REE"].diff()
df_medias["position_TEF"] = df_medias["signals_TEF"].diff()

df_medias["position_BKT"][df_medias["position_BKT"] == 0] = None 
df_medias["position_CLNX"][df_medias["position_CLNX"] == 0] = None 
df_medias["position_ENG"][df_medias["position_ENG"] == 0] = None 
df_medias["position_REE"][df_medias["position_REE"] == 0] = None 
df_medias["position_TEF"][df_medias["position_TEF"] == 0] = None 

df_medias["position_BKT"].fillna(method =  "ffill", inplace = True)
df_medias["position_CLNX"].fillna(method =  "ffill", inplace = True)
df_medias["position_ENG"].fillna(method =  "ffill", inplace = True)
df_medias["position_REE"].fillna(method =  "ffill", inplace = True)
df_medias["position_TEF"].fillna(method =  "ffill", inplace = True)
...
algo_1 = algo_sharpe_medias(market = "IBEX", algo_tag = algo_tag, rebal_period = 7) 
z = algo_1.dayly_proc()

file = open("C:/Users/Rodrigo/MASTER_IA/algo_batch/log_ejecuciones.txt", "a")
file.write("algo_3_IBEX se ha ejecutado a las " + str(datetime.now()) + "\n")
file.close()
