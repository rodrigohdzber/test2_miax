...
holdeo = ["AD.AS","AXAF.PA", "SCHN.PA", "TEF.MC", "VIV.PA"]
...
df_medias["short_AD.AS"] = MME(df_holdeo_sin_na.iloc[:,0],50)
df_medias["long_AD.AS"] = MME(df_holdeo_sin_na.iloc[:,0],100)
df_medias["short_AXAF.PA"] = MME(df_holdeo_sin_na.iloc[:,1],50)
df_medias["long_AXAF.PA"] = MME(df_holdeo_sin_na.iloc[:,1],100)
df_medias["short_SCHN.PA"] = MME(df_holdeo_sin_na.iloc[:,2],50)
df_medias["long_SCHN.PA"] = MME(df_holdeo_sin_na.iloc[:,2],100)
df_medias["short_TEF.MC"] = MME(df_holdeo_sin_na.iloc[:,3],50)
df_medias["long_TEF.MC"] = MME(df_holdeo_sin_na.iloc[:,3],100)
df_medias["short_VIV.PA"] = MME(df_holdeo_sin_na.iloc[:,4],50)
df_medias["long_VIV.PA"] =MME(df_holdeo_sin_na.iloc[:,4],100)

df_medias["signals_AD.AS"] = np.where(df_medias["short_AD.AS"] > df_medias["long_AD.AS"], 1, 0)
df_medias["signals_AXAF.PA"] = np.where(df_medias["short_AXAF.PA"] > df_medias["long_AXAF.PA"], 1, 0)
df_medias["signals_SCHN.PA"] = np.where(df_medias["short_SCHN.PA"] > df_medias["long_SCHN.PA"], 1, 0)
df_medias["signals_TEF.MC"] = np.where(df_medias["short_TEF.MC"] > df_medias["long_TEF.MC"], 1, 0)
df_medias["signals_VIV.PA"] = np.where(df_medias["short_VIV.PA"] > df_medias["long_VIV.PA"], 1, 0)

df_medias["position_AD.AS"] = df_medias["signals_AD.AS"].diff()
df_medias["position_AXAF.PA"] = df_medias["signals_AXAF.PA"].diff()
df_medias["position_SCHN.PA"] = df_medias["signals_SCHN.PA"].diff()
df_medias["position_TEF.MC"] = df_medias["signals_TEF.MC"].diff()
df_medias["position_VIV.PA"] = df_medias["signals_VIV.PA"].diff()

df_medias["position_AD.AS"][df_medias["position_AD.AS"] == 0] = None 
df_medias["position_AXAF.PA"][df_medias["position_AXAF.PA"] == 0] = None 
df_medias["position_SCHN.PA"][df_medias["position_SCHN.PA"] == 0] = None 
df_medias["position_TEF.MC"][df_medias["position_TEF.MC"] == 0] = None 
df_medias["position_VIV.PA"][df_medias["position_VIV.PA"] == 0] = None 

df_medias["position_AD.AS"].fillna(method =  "ffill", inplace = True)
df_medias["position_AXAF.PA"].fillna(method =  "ffill", inplace = True)
df_medias["position_SCHN.PA"].fillna(method =  "ffill", inplace = True)
df_medias["position_TEF.MC"].fillna(method =  "ffill", inplace = True)
df_medias["position_VIV.PA"].fillna(method =  "ffill", inplace = True)
...
algo_1 = algo_sharpe_medias(market = "EUROSTOXX", algo_tag = algo_tag, rebal_period = 7) 
z = algo_1.dayly_proc()

file = open("C:/Users/Rodrigo/MASTER_IA/algo_batch/log_ejecuciones.txt", "a")
file.write("algo_3_EUROSTOXX se ha ejecutado a las " + str(datetime.now()) + "\n")
file.close()